...
public class IMAPStore {
    ...
    protected IMAPStore(Session session, URLName url, String name, boolean isSSL) {
        ...
        if (this.enableSASL) {
            ...
            if (s != null && s.length() > 0) {
                ...
                v.copyInto((Object[]) this.saslMechanisms);
            }
        }
    ...
    private void refreshPassword() {
        ...
        PasswordAuthentication pa = this.session.requestPasswordAuthentication(inetAddress, this.port, this.name, null, this.user);
  ...
  protected IMAPFolder newIMAPFolder(String fullName, char separator, Boolean isNamespace) {
    ...
    public synchronized Quota[] getQuota(String root) throws MessagingException {
        ...
        try {
            ...
            throw new MessagingException("QUOTA not supported", (Exception) bex);
        } catch (ConnectionException cex) {
            throw new StoreClosedException(this, cex.getMessage());
        } catch (ProtocolException pex) {
            throw new MessagingException(pex.getMessage(), (Exception) pex);
    ...
    public void idle() throws MessagingException {
        ...
        try {
            ...
            throw new MessagingException("IDLE not supported", (Exception) bex);
        } catch (ConnectionException cex) {
            throw new StoreClosedException(this, cex.getMessage());
        } catch (ProtocolException pex) {
            throw new MessagingException(pex.getMessage(), (Exception) pex);
