...
public class SearchSequence {
    ...
    public Argument generateSequence(SearchTerm term, String charset) throws SearchException, IOException {
        ...
        if (term instanceof javax.mail.search.SentDateTerm) {
            return sentdate((DateTerm) term);
        }
        if (term instanceof javax.mail.search.ReceivedDateTerm) {
    ...
    protected Argument or(OrTerm term, String charset) throws SearchException, IOException {
        ...
        if (terms.length > 2) {
            Object object = terms[0];

            for (int i = 1; i < terms.length; i++) {
                object = new OrTerm((SearchTerm) object, terms[i]);
            }
            term = (OrTerm) object;
